...
CALLBACKS = [
    ...
    {"name": "cmd_help", "args": None,
        "hint": "Whenever the user seems to be asking for how the bot works"},
    ...
    {"name": "cmd_add_expert", "args": [
        "username", "description"], "hint": "When a user wants to add another one as an expert"},
    {"name": "cmd_introduce", "args": [
        "description"], "hint": "When user is introducing themselves and stating what they know."},
    {"name": "cmd_answer", "args": ["answer"],
        "hint": "When user answers a question"},
    {"name": "cmd_query", "args": [
        "query"], "hint": "When user asks a question or orders a task"},
    {"name": "cmd_remove_tool", "args": [
        "tool_name"], "hint": "When user asks to remove a tool"}
]
...
def process_message(update, context):
    ...
    if bot_username == MAIN_BOT_NAME or chat_text.startswith("/"):
        context.bot.send_message(
            chat_id=chat_id, text=f"Sorry, I'm just meant to receive /start and /token commands.")
    ...
    logging.info(" ---> DONE RESPONSE")
    if chat_state == "waiting_for_answer":
        context.bot.send_message(
            chat_id=chat_id, text=f"Thanks for your help {name} :).")
        cmd_answer(update, context, chat_text)
        return
    elif chat_state == "waiting_for_clarification":
        context.bot.send_message(
            chat_id=chat_id, text=f"Thank you for clarifying that {name} :). I'll send you a message when I have an answer.")
        cmd_answer(update, context, chat_text)
        return
    elif chat_state == "waiting_for_feedback":
        context.bot.send_message(
            chat_id=chat_id, text=f"Thank you for your feedback {name} :).")
        cmd_answer(update, context, chat_text)
        return
    ...
    try:
        llm_opinion = router_chain.run(
            {"input": chat_text, "callbacks": CALLBACKS})
    ...
    try:
        ...
        app.log.error(e)
        context.bot.send_message(
            chat_id=chat_id, text=f"Error: {e}", parse_mode=ParseMode.MARKDOWN,)
        return
    ...
    if debug:
        ...
        context.bot.send_message(
            chat_id=main_chat_id, text=f"Hi, router agent here. I got the message: \n'''{chat_text}'''\n  and I matched it to the command: \n{callback_name} \n with args: \n{callback_args}")
    ...
    try:
        ...
        app.log.error(e)
        context.bot.send_message(
            chat_id=chat_id, text=f"Error trying to fetch the mapped command: {e}")
        return
    ...
    if callback_args is None:
        ...
        try:
            ...
            app.log.error(e)
            context.bot.send_message(
                chat_id=chat_id, text=f"Error trying to execute the mapped command. \n Error: {e}")
...
def message_handler(event, context):
    ...
    if community_id not in bots_db.values():
        ...
        logging.info(f"Unauthorized Bot Token: {community_id}")
        return {
            "statusCode": 401,
            "body": json.dumps({
    ...
    try:
        ...
        processed_messages = set(json.loads(
    ...
    dispatcher.add_handler(CommandHandler(
        'reset_defaults', cmd_reset_defaults))
    ...
    dispatcher.add_handler(CommandHandler(
        'show_pending_queries', cmd_show_pending_queries))
    dispatcher.add_handler(CommandHandler(
        'delete_pending_queries', cmd_delete_pending_queries))
    dispatcher.add_handler(CommandHandler('add_moderator', cmd_add_moderator))
    dispatcher.add_handler(CommandHandler("ask", cmd_ask))
    dispatcher.add_handler(CommandHandler(
        'enable_feedback', cmd_enable_feedback))
    dispatcher.add_handler(CommandHandler(
        'disable_feedback', cmd_disable_feedback))
    ...
    dispatcher.add_handler(CommandHandler(
        'deregister_bot', cmd_deregister_bot))
    dispatcher.add_handler(CommandHandler(
        'add_global_mod', cmd_add_global_moderator))
    dispatcher.add_handler(CommandHandler(
        'remove_global_mod', cmd_remove_global_moderator))
    dispatcher.add_handler(CommandHandler(
        'show_global_mods', cmd_show_global_moderators))
    dispatcher.add_handler(MessageHandler(
        Filters.entity(MessageEntity.URL), handle_url))
    ...
    try:
        ...
        print(e)
        logging.info("EXCEPTION MESSAGE - MAIN",str(e))
